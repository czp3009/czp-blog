...
字面量\(literal\)是写在源代码中的表示固定值的符号\(token\). 平时说的硬编码\(hard coding\)大多数时候就是在说字面量\(有时指常量\). 举个例子
...
将变量赋值为另一个字面量并没有修改原来的字面量. 修改字面量说的是这种情况
...
`0x00000690` 是之前在 `readelf -S` 命令中可以看到的 `.rodata` 节的起始位置, `01000200` 是与操作系统有关的魔数, 在 linux x86\_64 平台固定为这个值, 之后就是用户代码里写的字符串字面量\(因为是字符串, 最后还有个 \0\).
