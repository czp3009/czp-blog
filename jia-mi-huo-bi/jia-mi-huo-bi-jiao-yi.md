...
由于没有交割日, 永续合约的价格通过另一个机制来保持合约价格趋向现货价格. 交易所会在每天确定的几个固定时间点, 例如每隔八小时进行一次判定, 如果合约标记价格大于现货价格则看涨用户要补一定资金给看跌用户\(在仓里扣\), 如果合约标记价格小于现货价格则看跌用户要补一定资金给看涨用户. 这样就可以对偏离现货价格的一方实施惩罚迫使合约价格逼近现货价格.
